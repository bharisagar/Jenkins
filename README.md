# Jenkins Day 1 and Day 2 Student Notes

This repository contains beginner-friendly Jenkins notes and practical steps for DevOps students who already know basic Linux, Git, and Docker.

## Course Context

- Class timing: 7:00 AM to 8:30 AM
- Audience: DevOps beginners
- Prerequisites: Linux basics, Git basics, Docker basics
- Goal: Learn Jenkins from scratch using practical CI/CD examples
...
- Jenkins security with Matrix-based security
- Jenkins credentials
- Jenkins freestyle jobs
- Jenkins build logs
- Basic Git integration with Jenkins
...
Jenkins can run shell commands, access source code, store secrets, and deploy applications. Because of this, Jenkins must be secured.
...
If Matrix-based security is missing, install:
...
| `student1` | `Overall/Read`, `Job/Read`, `Job/Build`, `Job/Workspace` |
| `readonly` | `Overall/Read`, `Job/Read`, `View/Read` |
| `anonymous` | No permissions |

Important:

Always make sure at least one admin user has `Overall/Administer` before saving.

## 12. Jenkins Credentials

Credentials store secrets safely in Jenkins.
...
| --- | --- |
| Username with password | Git HTTPS token, Docker Hub login |
| Secret text | API token |
| SSH Username with private key | SSH Git clone |
| Secret file | kubeconfig or service account file |

Example Git HTTPS credential:
...
ID: git-token
Description: Git token for Jenkins
```

## 13. Jenkins Build Logs
...
Ubuntu apt Jenkins build log path:

```text
/var/lib/jenkins/jobs/Project1/builds/2/log
```

Docker Jenkins build log path:

```text
/var/jenkins_home/jobs/Project1/builds/2/log
```
...
## 15. Jenkins Project Types

| Project Type | Meaning | Use Case |
| --- | --- | --- |
| Freestyle | Basic UI-based job | Beginner jobs, simple automation |
| Pipeline | Groovy-based Jenkinsfile | Real CI/CD pipelines |
| Multibranch Pipeline | Runs Jenkinsfile for multiple branches | Large projects with feature branches |

Simple summary:

- Freestyle means click-based job setup.
- Pipeline means Jenkinsfile-based automation.
- Multibranch Pipeline means Jenkinsfile automation for many branches.

## 16. Day 2 Practical Project: Project1
...
2. Screenshot of credentials ID.
3. Screenshot of Project1 Git SCM configuration.
4. Screenshot of successful Console Output.
5. Screenshot of build log path from Jenkins filesystem.
6. One failed build screenshot and explanation.
...
### Q3. What is Matrix-based security?

Matrix-based security is a Jenkins authorization method where users or groups are added to a permission table and access is given using checkboxes.

### Q4. What are Jenkins credentials?

Jenkins credentials store secrets such as passwords, tokens, private keys, and secret files safely.

### Q5. What is a freestyle job?

A freestyle job is a basic Jenkins job configured from the UI. It can pull code from Git and run shell commands.

### Q6. Where are Jenkins build logs stored?
...
### Q7. What is the difference between freestyle and pipeline?

Freestyle jobs are configured through the Jenkins UI. Pipeline jobs are written as code using a Jenkinsfile.

## Recommended 7-Day Jenkins Flow

| Day | Topic |
| --- | --- |
| Day 1 | Jenkins introduction, CI/CD, architecture, installation |
| Day 2 | Security, credentials, logs, freestyle Git job |
| Day 3 | Git webhooks and automatic build triggers |
| Day 4 | Jenkins pipeline and Jenkinsfile basics |
| Day 5 | Jenkins with Docker |
